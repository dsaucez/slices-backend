...
type GetAuthenticationParams struct {
	// RequestedUri URI from where the client comes from, encoded in BASE64
	RequestedUri string `form:"requested_uri" json:"requested_uri"`
}
...
// GetRedirectCallParams defines parameters for GetRedirectCall.
type GetRedirectCallParams struct {
	// Code authorization code generated by OIDC
	Code string `form:"code" json:"code"`

	// State URI to which the client must be redirected to, encoded in BASE64
...
type ServerInterface interface {
	...
	// Finalize the auhtentication with OIDC based on authentication code
	// (GET /redirect-call)
	GetRedirectCall(w http.ResponseWriter, r *http.Request, params GetRedirectCallParams)
...
func (siw *ServerInterfaceWrapper) GetAuthentication(w http.ResponseWriter, r *http.Request) {
	...
	// ------------- Required query parameter "requested_uri" -------------

	if paramValue := r.URL.Query().Get("requested_uri"); paramValue != "" {

	} else {
		siw.ErrorHandlerFunc(w, r, &RequiredParamError{ParamName: "requested_uri"})
		return
	}

	err = runtime.BindQueryParameter("form", true, true, "requested_uri", r.URL.Query(), &params.RequestedUri)
	if err != nil {
		siw.ErrorHandlerFunc(w, r, &InvalidParamFormatError{ParamName: "requested_uri", Err: err})
		return
	}
...
func (siw *ServerInterfaceWrapper) GetRedirectCall(w http.ResponseWriter, r *http.Request) {
	...
	// Parameter object where we will unmarshal all parameters from the context
	var params GetRedirectCallParams
	...
	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.GetRedirectCall(w, r, params)
	}))
